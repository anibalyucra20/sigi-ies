...
if (!verificar_sesion($conexion)) {
    ...
    if ($rb_permiso['id_rol'] != 1 || $rb_usuario['estado'] == 0) {
        ...
        //actualizar
        $consulta = "UPDATE sigi_capacidades SET id_unidad_didactica='$unidad_didactica', id_competencia='$competencia', codigo='$codigo', descripcion='$descripcion' WHERE id=$id";
        $ejec_consulta = mysqli_query($conexion, $consulta);
        if ($ejec_consulta) {
            echo "<script>
			alert('Registro Actualizado de manera Correcta');
			window.location= '../capacidades';
		</script>
	";
        } else {
            echo "<script>
