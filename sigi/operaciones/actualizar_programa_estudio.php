...
if (!verificar_sesion($conexion)) {
    ...
    if ($rb_permiso['id_rol'] != 1 || $rb_usuario['estado']==0) {
        ...
        $perfil_egresado = $_POST['perfil_egreso'];

        //actualizar
        $sql = "UPDATE sigi_programa_estudios SET codigo='$codigo', tipo='$tipo', plan_estudios='$plan_estudios', nombre='$nombre', resolucion='$resolucion', perfil_egresado='$perfil_egresado' WHERE id=$id";
        $ejec_consulta = mysqli_query($conexion, $sql);
        if ($ejec_consulta) {
            echo "<script>
			alert('Registro Actualizado de manera Correcta');
			window.location= '../programa_estudios';
		</script>
	";
        } else {
            echo "<script>
