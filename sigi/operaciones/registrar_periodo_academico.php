...
if (!verificar_sesion($conexion)) {
    ...
    if ($rb_permiso['id_rol'] != 1 || $rb_usuario['estado'] == 0) {
        ...
        $insertar = "INSERT INTO sigi_periodo_academico (nombre, fecha_inicio, fecha_fin, director, fecha_actas) VALUES ('$periodo','$fecha_inicio','$fecha_fin', '$director', '$fecha_actas')";
        $ejecutar_insetar = mysqli_query($conexion, $insertar);
        if ($ejecutar_insetar) {
            echo "<script>
                alert('Registro Existoso');
                window.location= '../periodo_lectivo'
    			</script>";
        } else {
            echo "<script>
			alert('Error al registrar Periodo Léctivo');
			window.history.back();
				</script>
			";
        };
