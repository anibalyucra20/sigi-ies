...
if (!verificar_sesion($conexion)) {
    ...
    <a href='../../include/cerrar_sesion_tutoria.php'>Cerrar Sesión</a><br>
        ...
        $r_b_tutoria = mysqli_fetch_array($b_tutoria);
            ...
            $id_tutoria_actual= $_POST['data2'];

            $consulta = "UPDATE tutoria_estudiantes SET id_programacion_tutoria='$id_tutoria' WHERE id=$id_tutoria_est";
            $ejec_consulta = mysqli_query($conexion, $consulta);
            if ($ejec_consulta) {
                echo "<script>
            alert('Datos Modificados Correctamente');
			window.location= '../tutoria_estudiantes?data=".base64_encode($id_tutoria_actual)."'
		</script>
	";
            } else {
                echo "<script>
